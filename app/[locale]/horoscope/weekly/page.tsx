...
import { generateWeeklyHoroscope } from '@/lib/horoscope-content-enhanced';
...
export default async function WeeklyHoroscopePage({ params, searchParams }: PageProps) {
  ...
  const horoscope = generateWeeklyHoroscope(selectedSignId, weekStart, locale);

  return (
    <div className="container mx-auto py-12 px-4">
      <div className="space-y-8">
        <div className="text-center space-y-4">
          <h1 className="text-4xl font-bold">{t('weekly')}</h1>
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                {zodiacSigns.map((sign) => (
                  <Link
                    key={sign.id}
                    href={`/horoscope/weekly?sign=${sign.id}`}
                    className="group"
                  >
